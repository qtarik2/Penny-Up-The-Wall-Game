...
public class lb_BirdController : MonoBehaviour {
	...
	void Start () {
		...
		for(int i=0;i<myBirds.Length;i++){
			...
			myBirds[i] = Instantiate (bird,transform.position,Quaternion.identity) as GameObject;
		...
		for(int i=0;i<3;i++){
			featherEmitters[i] = Instantiate (fEmitter,transform.position,Quaternion.identity) as GameObject;
